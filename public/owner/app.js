let owner = JSON.parse(localStorage.getItem('owner') || 'null');
let socket = null;
...
async function register() {
  ...
  const res = await fetch('/api/owner/register', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, phone, password }) });
...
async function login() {
  ...
  const res = await fetch('/api/owner/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ phone, password }) });
  const data = await res.json();
  if (!res.ok) return showError(data.error);
  owner = data;
  localStorage.setItem('owner', JSON.stringify(owner));
...
async function loadDashboard() {
  const res = await fetch(`/api/owner/${owner.id}/dashboard`);
...
async function resolveSos(id) {
  await fetch(`/api/owner/${owner.id}/sos/${id}/resolve`, { method: 'POST' });
  loadDashboard();
}
...
async function addTaxi() {
  ...
  if (!plate) return;
  const res = await fetch(`/api/owner/${owner.id}/taxis`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ plate }) });
  const data = await res.json();
...
async function loadTaxis() {
  ...
  // keep the assign-driver dropdowns in sync
...
async function assignTaxi(driverId, taxiId) {
  await fetch(`/api/owner/${owner.id}/drivers/${driverId}/assign`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ taxi_id: taxiId || null }) });
  loadTaxis();
}

async function toggleStatus(driverId, status) {
  await fetch(`/api/owner/${owner.id}/drivers/${driverId}/status`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) });
  loadDrivers();
}
...
async function addDriver() {
  ...
  const res = await fetch(`/api/owner/${owner.id}/drivers`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, phone, license_no, pdp_no }) });
  const data = await res.json();
  if (!res.ok) return alert(data.error);
...
async function loadFeedback() {
  const res = await fetch(`/api/owner/${owner.id}/feedback`);
...
async function sendMessage() {
  ...
  await fetch(`/api/owner/${owner.id}/message`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ driver_id, text }) });
